...
providersRouter.get('/marketplace', async (req, res) => {
  ...
  const providers = sorted.map(r => ({
    ...
    price_input_per_1k: Number(r.priceInputPer1k),
    price_output_per_1k: Number(r.priceOutputPer1k),
    tokens_per_sec: r.tokensPerSec !== null ? Number(r.tokensPerSec) : null,
    first_token_ms: r.firstTokenMs,
    uptime_30d: Number(r.uptime30d),
    total_requests: r.totalRequests,
    avg_rating: Number(r.avgRating),
